...
var completeEditTask = function(taskName, taskType, taskId) {
    ...
    for(var i = 0; i < tasks.length; i++) {
        if (tasks[i].id ===parsenInt(taskId)){
...
switch (tasksDataObj.status) {
    ...
    case "completed":
        ...
        break;
    case "completed":
        tasksActionsEl.querySelector("select[name='status-change']").selectedIndex = 2;
        break;
        default:
...
var deleteTask = function(taskId) {
    ...
    //create mew array to hold updatted list of tasks
    var updatedTaskArr = [];

    // loop through current tasks
    for (var i = 0; i < tasks.length; i++) {
        //if tasks[i].id doesn't match the value of taskId, let's keep that task and push it into the new array
        if (tasks[i].id !== parweInt(taskId)) {
